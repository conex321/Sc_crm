...
async function ingest(c: DialpadCall): Promise<"new" | "duplicate"> {
  ...
  const activityRows = await sql`
    insert into public.activities (channel, direction, summary, occurred_at, account_id, contact_id)
    values ('call', ${c.direction}, ${summary}, ${occurredAt}, ${match?.accountId ?? null}, ${match?.contactId ?? null})
    returning id
  `;
...
async function main() {
  ...
  console.error(`Done · pages=${pages} pulled=${pulled} inserted=${inserted} duplicates=${duplicates}`);
}
